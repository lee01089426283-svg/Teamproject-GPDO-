...
class GPDOAnalyzer:
    ...
    Parameters
    ----------
    gpdo_dir : XML 파일이 있는 폴더 (하위 폴더 재귀 탐색)
    wafer_id : 처리할 웨이퍼 ID (XML 내부 TestSiteInfo.Wafer 기준)
    """
    ...
    def __init__(self, gpdo_dir: str, wafer_id: str = "D08"):
        ...
        # ① 폴더 + 하위 폴더까지 전체 XML 수집 (중복 제거)
        all_xml = sorted(set(
            glob.glob(os.path.join(gpdo_dir, "*.xml")) +
            glob.glob(os.path.join(gpdo_dir, "**", "*.xml"), recursive=True)
        ))
        ...
        # ③ wafer_id 필터 (XML 내부 TestSiteInfo.Wafer 기준)
        self.xml_files = [
            f for f in gpdo_all
    ...
    def _match_wafer(xml_path: str, wafer_id: str) -> bool:
        """XML 내부 TestSiteInfo.Wafer 속성으로 wafer_id 일치 여부 확인."""
        try:
    ...
    def run(self, save_dir: str) -> list:
        """
        전체 XML 처리 → 개별 그래프 저장 → 히트맵 저장.

        Parameters
        ----------
        save_dir : PNG 저장 루트 폴더

        Returns
        -------
        list of dict  (각 다이의 분석 결과)
        """
        ...
        for key, title, unit, cmap in self.HEATMAP_PARAMS:
            HeatmapPlotter.plot(
                self.results, key, title, unit, cmap,

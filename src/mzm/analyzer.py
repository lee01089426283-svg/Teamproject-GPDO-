...
from config import RES_DIR, PROJECT_NAME, WAFER_IDS, PROJECT_NAMES, _DATA_ROOT, _get_wafer_ids
from src.mzm.parser import MZMParser
from src.mzm.plotter import Plotter
from src.mzm.csv import generate_csv, generate_total_csv
from src.heatmap_plotter import HeatmapPlotter
...
def _png_dir(wafer: str, date: str, project_name: str = '') -> str:
    return os.path.join(RES_DIR, 'png', 'MZM', project_name, wafer, date)

def _heatmap_dir(wafer: str, date: str, project_name: str = '') -> str:
...
class MZMAnalyzer:
    def run_wafer(self, wafer_id: str, project_name: str = PROJECT_NAME) -> tuple[str, list]:
        ...
        for date, xml_file in xml_files:
            out = Plotter.plot(
                xml_file,
                save_dir=_png_dir(wafer_id, date, project_name),
                verbose=True,
            )
            if out:
                pngs.append(out)
        ...
        self._plot_heatmaps_by_timestamp(ts_data, wafer_id, project_name)
    ...
    def run(self, verbose: bool = True) -> tuple[dict, dict]:
        ...
        for project_name in PROJECT_NAMES:
            ...
            for wafer in wafer_ids:
                print(f'\n{"─"*50}')
                print(f'  Project: {project_name}  |  Wafer: {wafer}')
                print(f'{"─"*50}')

                csv_path, pngs = self.run_wafer(wafer, project_name=project_name)
    ...
    @staticmethod
    def _plot_heatmaps_by_timestamp(ts_data: dict, wafer: str,
                                    project_name: str = PROJECT_NAME) -> None:
        for date, rows in ts_data.items():
            if not rows:
                continue
            save_dir = _heatmap_dir(wafer, date, project_name)
